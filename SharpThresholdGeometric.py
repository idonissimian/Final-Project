...
def add_sparses_to_dense_tree(g_two_tags, net, groups_on_grid, friends_dict):
    ...
    for k in friends_dict.keys():
        ...
        for dense_sq in list_of_friends:
            if are_squares_connected(net, curr_sparse, dense_sq)==True:
...
def travel_squares_by_times_and_build_cycle(net, g_two_tags, groups_on_grid, counter, all_in_out_times):
    ...
    for i in range(counter):
        ...
        if i == 0:
            ...
            print("path:", path)
            curr_sq_i['arr_points'] = np.delete(curr_sq_i['arr_points'], np.where(curr_sq_i['arr_points'] == curr_sq_i['arr_points'][i]))
        ...
        if i + 1 == counter and index_point_i == temp_sq_num_of_first:
            for j in range(len(curr_sq_i['arr_points'])):
                ...
                print("path:", path)
                curr_sq_i['arr_points'] = np.delete(curr_sq_i['arr_points'], np.where(curr_sq_i['arr_points'] == curr_sq_i['arr_points'][0]))
        index_i_plus_one, curr_sq_i_plus_one = square_of_counter(groups_on_grid, i + 1, all_in_out_times)

        # If curr_sq_i is sparse
        if curr_sq_i['is_den'] == False:
            ...
            print("path:", path)
            curr_sq_i_plus_one['arr_points'] = np.delete(curr_sq_i_plus_one['arr_points'], np.where(curr_sq_i_plus_one['arr_points'] == next_point))
...
def get_first_point_connected(net, curr_point_serial_number, next_sq):
    for next_point in next_sq['arr_points']:
        if net.is_at_edge_by_points(net.edges, curr_point_serial_number, next_point.serial_number) == True:
...
def main():
    ...
    nx.draw_networkx_edge_labels(g_tag_spanning_tree, pos_g_tag_spanning_tree, label_pos=0.3, edge_labels=labels_g_tag_spanning_tree)
    plt.savefig("g_tag_spanning_tree_drawing.png")
    plt.show()
    print("========================================================================================")
